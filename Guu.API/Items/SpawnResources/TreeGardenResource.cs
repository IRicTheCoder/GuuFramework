...
{
	/// <summary>
	/// This is the base class to make garden spawn resources (trees)
	/// </summary>
	public abstract class TreeGardenResource : SpawnResourceItem
	{
		...
		protected virtual Vector3 TreeScale { get; } = Vector3.one * 0.7f;
		...
		protected virtual Vector3 LeavesScale { get; } = Vector3.one;

		/// <summary>The base item to use when creating the one</summary>
		protected virtual GameObject BaseItem => IsDeluxe ?
			GameContext.Instance.LookupDirector?.GetResourcePrefab(SpawnResource.Id.POGO_TREE_DLX) ?? SRObjects.Get<GameObject>("treePogo04") :
			GameContext.Instance.LookupDirector?.GetResourcePrefab(SpawnResource.Id.POGO_TREE) ?? SRObjects.Get<GameObject>("treePogo01");
