...
namespace Guu.API.Identifiables
{
	...
	public abstract class Largo : IdentifiableItem
	{
		...
		///<summary>The definitions for each largo</summary>
		private Dictionary<Identifiable.Id, SlimeDefinition> Definitions { get; } = new Dictionary<Identifiable.Id, SlimeDefinition>();
		...
		protected override void Build()
		{
			...
			//{
				// Make new Definition
				SlimeDefinition other = SlimeUtils.GetDefinitionByID(Identifiable.Id.ROCK_SLIME);
				...
				//+ CONFIGURE DIETS
				// Slime A diet
				Definition.Diet.EatMap.Add(new SlimeDiet.EatMapEntry
				{
					eats = SlimeUtils.SlimeToPlort(other.IdentifiableId),
					becomesId = newDef.IdentifiableId,
					driver = SlimeEmotions.Emotion.AGITATION,
					minDrive = 0.5f
				});
				
				// Slime B diet
				other.Diet.EatMap.Add(new SlimeDiet.EatMapEntry
				{
					eats = SlimeUtils.SlimeToPlort(Definition.IdentifiableId),
					becomesId = newDef.IdentifiableId,
					driver = SlimeEmotions.Emotion.AGITATION,
					minDrive = 0.5f
				});
				...
				Definitions.Add(newDef.IdentifiableId, newDef);
			//}
		}
		...
		public override IdentifiableItem Register()
		{
			...
			foreach(Identifiable.Id id in Prefabs.Keys)
			{
				...
				SlimeRegistry.RegisterSlimeDefinition(Definitions[id]);
